...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const id = urlParams.get('id'); // Support ?id= parameter from daily operations
    
    if (viewId || id) {
        // View-only mode (support both ?view= and ?id=)
        const certificateId = viewId || id;
        console.log('📖 View-only mode for certificate ID:', certificateId);
    ...
    // Setup auto-update for birth date text in paragraph
    const birthDateTextInput = document.getElementById('birthDateText');
    if (birthDateTextInput) {
        birthDateTextInput.addEventListener('input', updateBirthDateInParagraph);
    }
    
    // Setup auto-update for birth date (date picker)
    const birthDateNew = document.getElementById('birthDateNew');
    if (birthDateNew) {
        birthDateNew.addEventListener('change', function() {
            // Format date and update the text input
            if (this.value) {
                const date = new Date(this.value);
                const formatted = formatDateToText(date);
                const birthDateTextInput = document.getElementById('birthDateText');
                if (birthDateTextInput) {
                    birthDateTextInput.value = formatted;
                    updateBirthDateInParagraph();
                }
            }
        });
    }
...
// Update birth date in paragraph (already inline, so just for consistency)
function updateBirthDateInParagraph() {
    console.log('Birth date updated in paragraph');
}

// Format date to readable text format
